    ...
    .defines(function() {
        ...
        var Valerie = ig.GameExtended.extend({
            ...
            initUI: function() {
                ...
                this.ui.player = {};
                this.ui.player.healthBar = document.getElementById('ui-player-status-bars-health-bar');
                this.ui.player.energyBar = document.getElementById('ui-player-status-bars-energy-bar');
                this.ui.player.portrait = document.getElementById('ui-player-status-portrait-image');
            ...
            updateUI: function() {
                ...
                if (!this.paused && this.player) {
                    ...
                    var playerHealthPercentage = (this.player.health / this.player.healthMax);

                    var green = Math.floor(playerHealthPercentage * 255);
                    var red = Math.floor((1 - playerHealthPercentage) * 255);

                    this.updateHTMLStyle(this.ui.player.healthBar, 'width', playerHealthPercentage * 100 + '%');
                    this.updateHTMLStyle(this.ui.player.healthBar, 'background', 'rgb(' + red + ',' + green + ',0)');

                    if (playerHealthPercentage <= 0.5) this.updateHTMLStyle(this.ui.player.portrait, 'background-position', '-96px 0px');
                    if (playerHealthPercentage > 0.5) this.updateHTMLStyle(this.ui.player.portrait, 'background-position', '0px 0px');

                    var playerEnergyPercentage = (this.player.energy / this.player.energyMax);

                    this.updateHTMLStyle(this.ui.player.energyBar, 'width', playerEnergyPercentage * 100 + '%');

                }

            },

            updateHTMLStyle: function(element, style, value) {
            ...
            inputEnd: function() {
                ...
                ig.input.unbind(ig.KEY._1, 'shoot');
