ig.module(
    ...
    .defines(function() {
        ...
        var _c = ig.constants;
        var _ut = ig.utils;
        var _uti = ig.utils.intersection;
        var _utv2 = ig.utils.vector2;
         ...
         * <br>- shapes are extracted by {@link ig.GameExtended#loadLevel}, but the work is done by {@link ig.utils.tile.shapesFromCollisionMap}
         ...
         * @readonly
         */
        ig.utils.tile.defaultTileVerticesDef = {};
         ...
         * @readonly
         */
        ig.utils.tile.defaultTileSegmentsDef = {};
        ...
        ig.utils.tile.rebuild = function(collisionMap) {
            ...
            if (collisionMap instanceof ig.CollisionMap && collisionMap.tiledef !== _collisionMapTileDef) {
                ...
                for (var tileId in _collisionMapTileDef) {
                    ...
                    ig.utils.tile.shapeFromTile(tileId, _collisionMapTileDef);

                }

                // don't forget about solid

                ig.utils.tile.shapeFromTile(1, _collisionMapTileDef);
         ...
         **/
        ig.utils.tile.isTileWalkable = function(tileId) {
         ...
         * @returns {Boolean} whether tile is one-way
         **/
        ig.utils.tile.isTileWalkableStrict = function(tileId) {
         ...
         * @returns {Boolean} whether tile is one-way
         **/
        ig.utils.tile.isTileOneWay = function(tileId) {
         ...
         **/
        ig.utils.tile.isTileClimbable = function(tileId) {
         ...
         **/
        ig.utils.tile.isTileClimbableStairs = function(tileId) {
         ...
         **/
        ig.utils.tile.isTileClimbableOneWay = function(tileId) {
         ...
         **/
        ig.utils.tile.shapesFromCollisionMap = function(map, options) {
            ...
            if (map instanceof ig.CollisionMap) {
                ...
                for (iy = 0; iy < height; iy++) {
                    ...
                    for (ix = 0; ix < width; ix++) {
                        ...
                        shape = ig.utils.tile.shapeFromTile(map.data[iy][ix], map.tiledef);
                ...
                shapes.solids = shapes.solids.concat(ig.utils.tile.shapedTilesToShapes(solids, data, options));
                ...
                shapes.climbables = shapes.climbables.concat(ig.utils.tile.shapedTilesToShapes(climbables, data, options));
                ...
                shapes.oneWays = shapes.oneWays.concat(ig.utils.tile.shapedTilesToShapes(oneWays, data, options));
         ...
         * <span class="alert alert-info"><strong>Tip:</strong> when converting tiles to shapes, is usually better to call {@link ig.utils.tile.shapesFromCollisionMap}.</span>
         * @param {Array} tiles shaped tiles to convert
         * @param {Array} data 2d list of all tiles in map
         * @param {Object} options options object
         * @returns {Array} list of shapes
         **/
        ig.utils.tile.shapedTilesToShapes = function(tiles, data, options) {
            ...
            if (options.groupByTileId) {
                ...
                for (i = 0, il = ids.length; i < il; i++) {
                    ...
                    shapes = shapes.concat(ig.utils.tile.shapedTilesToShapes(group, data, options));
                ...
                if (options.rectangles) {
                    ...
                    while (tilePool.length > 0) {
                        ...
                        line = ig.utils.tile.findShapedTileLine(tilePool);
                        ...
                        while (stepped) {
                            ...
                            if (tileFrom) {
                                ...
                                if (index !== -1) {
                                    ...
                                    line = ig.utils.tile.findShapedTileLine(tilePool, false, index, length);
                    ...
                    for (j = 0, jl = rectangles.length; j < jl; j++) {
                        ...
                        for (i = 0, il = rectangle.length; i < il; i++) {
                            ...
                            vertices = vertices.concat(ig.utils.tile.getNonDuplicateSegmentVertices(rectangle[i], data, rectangle));

                        }

                        // vertices to contours

                        contours = contours.concat(ig.utils.tile.verticesToContours(vertices, options));
                ...
                // general shapes that may or may not be concave
                    ...
                    for (i = 0, il = tiles.length; i < il; i++) {
                        ...
                        vertices = vertices.concat(ig.utils.tile.getNonDuplicateSegmentVertices(tiles[i], data, tiles));

                    }

                    // vertices to contours

                    contours = ig.utils.tile.verticesToContours(vertices, options);
         ...
         **/
        ig.utils.tile.findShapedTileLine = function(tiles, horizontal, indexFrom, length) {
            ...
            while (stepped) {
                ...
                var tileNext = horizontal ? ig.utils.tile.stepShapedTileHorizontally(tiles, tileFrom) : ig.utils.tile.stepShapedTileVertically(tiles, tileFrom);
         ...
         **/
        ig.utils.tile.stepShapedTileHorizontally = function(tiles, tileFrom) {
         ...
         * @returns {Object} next tile, or current tile if next not found
         **/
        ig.utils.tile.stepShapedTileVertically = function(tiles, tileFrom) {
         ...
         **/
        ig.utils.tile.verticesToContours = function(vertices, options) {
         ...
         **/
        ig.utils.tile.shapeFromTile = function(tileId, tileDefs) {

            var i, il;
            var vertices = ig.utils.tile.verticesFromTile(tileId, tileDefs);
            var segments;
            ...
            if (vertices) {
                ...
                if (ig.utils.tile.defaultTileSegmentsDef[tileId]) {

                    segments = ig.utils.tile.defaultTileSegmentsDef[tileId];

                } else {

                    ig.utils.tile.defaultTileSegmentsDef[tileId] = segments = [];
         ...
         **/
        ig.utils.tile.verticesFromTile = function(tileId, tileDefs) {
            ...
            if (ig.utils.tile.defaultTileVerticesDef[tileId]) {

                vertices = ig.utils.tile.defaultTileVerticesDef[tileId];
                ...
                ig.utils.tile.defaultTileVerticesDef[tileId] = vertices;
         ...
         **/
        ig.utils.tile.getNonDuplicateSegmentVertices = function(tile, tileData, tilesAllowed) {
            ...
            for (i = 0, il = segments.length; i < il; i++) {
                ...
                if ((normal.x === 0 && normal.y !== 0) || (normal.x !== 0 && normal.y === 0)) {
                    ...
                    var touchingTiles = ig.utils.tile.getTouchingTilesByDirection(tile, normal, tileData, tilesAllowed);
                    ...
                    for (j = 0, jl = touchingTiles.length; j < jl; j++) {
                        ...
                        if (touchingTile.shape.vertices.length > 0) {
                            ...
                            overlap = ig.utils.tile.getSegmentOverlapWithTile(va, vb, normal, touchingTile);
         ...
         **/
        ig.utils.tile.getTouchingTilesByDirection = function(tile, direction, tileData, tilesAllowed) {
         ...
         **/
        ig.utils.tile.getSegmentOverlapWithTile = function(vaA, vbA, normalCompare, tile) {
